...
import com.dtl.gemini.widget.klineview.PrintUtil;
        ...
        try {
            ...
            for (int i = 0; i < dataList.size(); i++) {
                if (i + QUOTA_DAY5 <= dataList.size()) {
                    if (dataList.get(i + QUOTA_DAY5 - 1) != null && cacheList.subList(i, i + QUOTA_DAY5) != null) {
                        //priceMa5
                ...
                if (i + QUOTA_DAY10 <= dataList.size() && cacheList.subList(i, i + QUOTA_DAY10) != null) {
                    if (dataList.get(i + QUOTA_DAY10 - 1) != null) {
                        //priceMa10
                ...
                if (i + QUOTA_DAY30 <= dataList.size() && cacheList.subList(i, i + QUOTA_DAY30) != null) {
                    //priceMa30
                    if (dataList.get(i + QUOTA_DAY30 - 1) != null) {
        ...
        try {
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                for (int j = i + 1 - period; j <= i; j++) {
                    //n-1日
                    if (j < dataList.size() && dataList.get(j) != null)
    ...
    public static void initRSI(List<KData> dataList, int period1, int period2, int period3, boolean isEndData) {
        try {
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                dnRateSum = 0;
                dnRateCount = 0;
                if (i < dataList.size() && i >= period1 - 1) {
                    for (int x = i; x >= i + 1 - period1; x--) {
                        if (x < (dataList.size() - 1) && dataList.get(x) != null) {
