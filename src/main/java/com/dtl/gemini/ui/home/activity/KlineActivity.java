 ...
 * 行情
 **/
public class KlineActivity extends BaseAppActivity implements View.OnClickListener {
                ...
                public void onSuccess(Object str) {
                    if (currPriceTv != null) {
                        ...
                        if (huoBiBean != null && huoBiBean.getTick() != null) {
                            ...
                            String hight = DataUtil.doubleFour(huoBi.getHigh());
                            String low = DataUtil.doubleFour(huoBi.getLow());
                            String vol = DataUtil.transfers(context, huoBi.getVol());
                            String priceCny = DataUtil.doubleFour(huoBi.getClose() * MainActivity.usd);
                    ...
                    if (kineProgressBar != null) {
                        ...
                        if (huoBiBean.getData() != null && huoBiBean.getData().size() > 0) {
                            ...
                            for (int i = huoBiBean.getData().size() - 1; i > huoBiBean.getData().size() / 2 - 1; i--) {
                                ...
                                if (height == low || height < low) {
                                    if (height < open || height < close) {
                                        //最高价低于开盘或收盘，取两者最高
                                        if (open > close) {
                                            height = open;
                                        } else {
...
//    }
    ...
    private void showDialog(List<String> list) {
        SelectDialog dialog = new SelectDialog(this, list, select, 2);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.speed_open_fall_btn:
                if (DataUtil.isFastClick()) {
                    StoreUtils.init(this).setParameter(Constant.MODEL, "cfd");
    ...
    public static void ListSort(List<KData> list) {
        Collections.sort(list, new Comparator<KData>() {
            @Override
            public int compare(KData o1, KData o2) {
                try {
    ...
    @Override
    public void finish() {
//        StoreUtils.init(this).setParameter(Constant.CURRENCY, null);
        super.finish();
    }
}
