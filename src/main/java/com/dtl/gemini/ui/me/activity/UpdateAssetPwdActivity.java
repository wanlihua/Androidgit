 ...
 * 修改提币密码
 **/
public class UpdateAssetPwdActivity extends BaseAppActivity implements View.OnClickListener {
    ...
    public void initView() {
        setTitle(getResources().getString(R.string.update_asset_pwd));
        ...
        switch (view.getId()) {
            ...
            case R.id.confirm_btn:
                if (DataUtil.isFastClick()) {
                    ...
                    if (!RegularUtil.valiAssetPwd(pwd1)) {
    ...
    private void getCode(Context context) {
        try {
            Api.getInstance().sendUpdateExtractPasswordSmsCode(context).compose(RxSchedulers.io_main()).subscribe(new RxSubscriber<BaseBean>(context) {
                @Override
     ...
     * @param pwd
     */
    private void updateAssetPwd(Context context, String pwd, String code) {
        try {
            Api.getInstance().updateExtractPassword(context, pwd, code).compose(RxSchedulers.io_main()).subscribe(new RxSubscriber<BaseBean>(context) {
                @Override
                protected void _onNext(BaseBean bean) {
                    if (bean != null && bean.getStatus() == Constant.SUCCESS) {
